        ...
        {items.length === 0 ? (
          ...
          <div className="space-y-4">
            {items.map((it, idx) => (
              <div key={idx} className="flex items-center gap-3 border-b pb-3">
                <div className="w-16 h-16 bg-gray-100 flex items-center justify-center overflow-hidden rounded">
                  {it.productImage ? <img src={it.productImage} alt={it.productName || 'Product'} className="w-full h-full object-cover" /> : <div className="text-xs text-gray-400">No image</div>}
                </div>
            ...
            <div className="flex gap-2 mt-3">
              <button onClick={() => { alert('Checkout flow not implemented'); }} className="bg-sage text-white px-4 py-2 rounded">Checkout</button>
